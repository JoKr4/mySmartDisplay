...
void http_req_relais_states() {
    ...
    if(httpCode > 0) {
        if(httpCode == HTTP_CODE_OK) {
            ...
            // TODO LV_BTNMATRIX_CTRL_CHECKED in ui button matrix
...
void http_req_relais_toggle(uint32_t id) {
    ...
    char buffer[16];
    sprintf(buffer, "/relais%d/toggle", id);

    httpClient.begin("192.168.178.69", 8090, buffer);
